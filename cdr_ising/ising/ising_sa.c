...
#define  OPTS       ":b:c:C:e:Ef:hlLNpQrs:StTvw:W:y:"
...
/* move generation (in move.c)                                             */
  ...
  if (init_point_flag){
    ...
    *p_chisq = UnifStartingPoints(); /* Simply Better */
  }
...
void RestoreState(char *statefile, NucStatePtr state_ptr, double *p_chisq)
{
  ...
  move_state      *move_ptr;                       /* used to restore moves */
 ...
 * No, stats does not get allocated in StatRead.        */
  ...
  move_ptr = (move_state *)malloc(sizeof(move_state));
