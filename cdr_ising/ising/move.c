...
/* acc_tab is for statistics like acceptance ratio etc. */
  ...
  spin_weight dummy;
  size_arr = n_points*(sizeof(dummy.spin));
  nbytes = size_arr + sizeof(acc_tab.hits) + sizeof(acc_tab.success) +
    sizeof(acc_tab.theta_bar) + sizeof(n_points) + sizeof(nhits) + 
    sizeof(nsweeps) + sizeof(curr_cost);
...
void RestoreMoves(move_state *moveptr){
//  int i;
...
/* allocate buffer */
  ...
  int *spin_buf = (int *)buf_pointer;
  for (int i = 0 ; i<n_points; ++i) {
    spin_buf[i]=VERTEX_SPIN(i);
  } 
  buf_pointer += n_points*sizeof(int);
  PACK_BYTES_VAR(buf_pointer, acc_tab.hits);
  PACK_BYTES_VAR(buf_pointer, acc_tab.success);
  PACK_BYTES_VAR(buf_pointer, n_points);
  PACK_BYTES_VAR(buf_pointer, nhits);
  PACK_BYTES_VAR(buf_pointer, nsweeps);
  PACK_BYTES_VAR(buf_pointer, curr_cost);
  PACK_BYTES_VAR(buf_pointer, acc_tab.theta_bar);
...
void AcceptStateMsg(unsigned char **buf)
{
  ...
  int *spin_buf = (int *)buf_pointer;
  for (int i = 0 ; i<n_points; ++i) {
    VERTEX_SPIN(i) = spin_buf[i];
  } 
  buf_pointer += n_points*sizeof(int);
  PACK_BYTES_TO_VAR(acc_tab.hits, buf_pointer);
  PACK_BYTES_TO_VAR(acc_tab.success, buf_pointer);
  PACK_BYTES_TO_VAR(n_points, buf_pointer);
  PACK_BYTES_TO_VAR(nhits, buf_pointer);
  PACK_BYTES_TO_VAR(nsweeps, buf_pointer);
  PACK_BYTES_TO_VAR(curr_cost, buf_pointer);
  PACK_BYTES_TO_VAR(acc_tab.theta_bar, buf_pointer);
...
void FreeMoveState(move_state *move_stuff){
  return;
}

double GetThetaBar(){
  return acc_tab.theta_bar;
}
