...
extern int n_points;
extern int n_edges;
extern spin_weight *weights_and_spin_states;
extern int degree;
extern double curr_cost;
...
/**********************************************
* CalcDiff calculates the difference between  *
* the energy of the proposed and the old      *
* spin configuration                          *
***********************************************/
...
void StateRead(char *statefile, Opts *options, move_state *moveptr, double *state, 
    unsigned short *rand, double *delta);

void RestoreProlix();

move_state *MoveSave(void);
...
/* MACROS */
#define VERTEX_OFFSET(v) ((v) * (degree + 1))
#define VERTEX_SPIN(v) weights_and_spin_states[VERTEX_OFFSET(v)].spin
#define VERTEX_EDGE(v,i) weights_and_spin_states[VERTEX_OFFSET(v)+1+(i)].weight
