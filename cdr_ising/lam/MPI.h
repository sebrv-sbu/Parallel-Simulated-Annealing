...
extern int myid;                           /* id of local node (processor) */
extern int nnodes;        /* number of nodes (processors), also known as P */
extern int tuning;                           /* flag for switching on tuning mode */
extern int covar_index;    /* covariance sample index for tuning (in 'tau' units) */
extern int write_tune_stat;               /* how often to write tuning statistics */
extern int auto_stop_tune;       /* auto stop tune flag to stop tuning runs early */
extern int write_llog;                        /* flag for writing local log files */ 
extern int logging_mix;       /* flag for writing detailed logs on mixing process */
extern int lam_group_size;  /* The size of an MPI group. In future, may change to *
                      * array if we want multiple groups of different size */
extern int ngroups;        /* number of groups */
extern int *root_ids;              /* array for root ids necessary for prolix and *
                             * equilibration runs                          */

extern bool score_method;
extern int glob_interval;

extern MPI_Group world;
extern MPI_Comm *my_comm;
extern MPI_Group *my_group;
extern MPI_Group *local_groups;
extern MPI_Comm *local_comms;
extern MPI_Group root_group;
extern MPI_Comm root_comm;
extern int my_group_id;
