...
static uint8_t    success=0;                 /* number of successful moves */
...
static uint16_t *m_success;  /* Move Success needed in move.h */
 ...
 * that's why we want to prevent overflows here (hence the 'if')           */
    ...
    if (i % proc_tau == proc_tau-1){
      MPI_Allreduce(MPI_IN_PLACE,m_success,1,MPI_UINT16_T, MPI_SUM, MPI_COMM_WORLD);
...
/* global stats are calculated here */
  ...
  MPI_Allreduce(MPI_IN_PLACE, &mvs, 1, MPI_Meanvarisucc, MPI_Meanvarisucc_sum, 
		MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &success_initial, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
...
/* parallel code: pool statistics from all nodes */
  ...
  mvs.success = success;
    ...
    MPI_Meanvarisucc_sum, MPI_COMM_WORLD);
...
/* MPI status and handle (request) arrays */
  ...
  count_mix++;
  AssignDancePartner(nnodes, MPI_COMM_WORLD, exp((estimate_mean-energy)*S));
 ...
 * the receive now, check for reception after send (using MPI_Waitall)     */
      ...
      MPI_Irecv(recvbuf, size+sizeof(double), MPI_BYTE, dance_partner[myid],
		dance_partner[myid], MPI_COMM_WORLD, &request);
  } 
...
/* send messages to dance partners, if requested */
  ...
  for (i=0; i<nnodes; i++) 
    if ( (dance_partner[i] == myid) && (i != myid) ) {
      MPI_Send(sendbuf, (size+sizeof(double)),MPI_BYTE, i, myid, MPI_COMM_WORLD);
    } 
...
void WriteMixLog(double *node_prob, int* dance_partner)
{
  ...
  MPI_Bcast(&root_id, 1, MPI_INT, 0, MPI_COMM_WORLD);
  MPI_Gather(&root_id, 1, MPI_INT, global_group_ids, 1, MPI_INT, 0, MPI_COMM_WORLD);
  if (count_mix % glob_interval){ // if local mix
    MPI_Gather(&node_prob[myid], 1, MPI_DOUBLE, global_node_prob, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Gather(&dance_partner[myid],1,MPI_INT, global_dance_partner, 1, MPI_INT, 0, MPI_COMM_WORLD);
    global_mix=0;
  }

  else{ // if global mix 
    MPI_Bcast(&node_prob[root_id], 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&dance_partner[root_id],1,MPI_INT,0,MPI_COMM_WORLD);
...
void Meanvarisucc_MPI_Init(){
  ...
  MPI_Datatype types[3]={MPI_DOUBLE, MPI_DOUBLE, MPI_UINT8_T};
