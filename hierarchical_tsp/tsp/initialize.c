...
uint16_t *ReadStartingPoints(FILE *start_file){
  int skip=myid;
  uint16_t index=0;
  int city;
  char buff[MAX_RECORD];
  for (int i = 0 ; i < skip; ++i){fgets(buff,MAX_RECORD,start_file);}
  uint16_t *tour_pos_pointer = malloc(sizeof(*tour_pos_pointer)*ncities*2);
  fgets(buff,MAX_RECORD,start_file);
  char *token=strtok(buff, ", \t");

  while(token != NULL && index<ncities){
    city=atoi(token);

    if (city >= ncities){error("Start file contains too many cities\n");}
    if (city <0){error("Start file contains negative cities?\n");}

    tour_pos_pointer[ncities+(uint16_t)city]=index;
    tour_pos_pointer[index++]=(uint16_t)city;
    token=strtok(NULL, " \t");
  }
  if ((token=strtok(NULL," \t"))!=NULL){
    error("Start file contains too many cities\n");
  }
  if (index<ncities){error("Start file contains too few cities\n");}
  return tour_pos_pointer;
} 
