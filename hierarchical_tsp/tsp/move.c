...
static uint16_t *curr_tour_and_pos = NULL;

static uint16_t *curr_tour = NULL;  /* pointer to tour of city ids 
                                  in current order visited*/ 
static uint16_t *curr_position = NULL;  /* maintains the current tour position 
                               for each city*/
                    /*index is city id-1 element is curr_tour index */ 
static uint16_t swap[2];  /* array of indices of swapped elements in tour*/ 
static uint16_t *min_tour = NULL;  /* pointer to the minimum annealing tour 
...
{  /* begin tour_allocate */
    curr_tour_and_pos=malloc (sizeof (*curr_tour_and_pos) * 2* ncities);   
if (curr_tour_and_pos == NULL)
 ...
 if (tour_debug) 
{ /* begin debug */
      min_tour =  malloc (sizeof (*min_tour) * ncities);
   if (min_tour == NULL)
      return 1;
} /* end debug */
    return 0;
...
double InitFromStartPoints(FILE *start_tour_file){
  curr_tour_and_pos=ReadStartingPoints(start_tour_file);
  curr_tour=curr_tour_and_pos;
  curr_position=curr_tour_and_pos+ncities;
  tour_max = (double) (ncities-1);      /* max index into tour array */
  neighbor_max = (double) (ncities-2);  /* for now use all neighbors */
...
void SaveInitialState(void **p){
  *p=malloc(sizeof(*curr_tour_and_pos)*ncities*2);
  memcpy(*p, curr_tour_and_pos,sizeof(*curr_tour_and_pos)*ncities*2);
}

void RestoreInitialState(void *p){
  memcpy(curr_tour_and_pos, p, sizeof(*curr_tour_and_pos)*ncities*2);
  free(p);
  curr_cost = TourCost(curr_tour);
}
