 ...
 * parsing a la Yogi for the ising_sa program                       *
 *                                                                *
 ******************************************************************/
...
#define  OPTS       ":bc:C:e:Ef:hlLNpQrs:StTvw:W:y:"
...
"Usage: ising_sa.mpi [-C <covar_ind>]  [-e <freeze_crit>]\n"
...
"Usage: ising_sa.mpi [options] <infile>\n\n"
...
"  -s <start_file>     set which file to read for starting points\n"
...
"Please report bugs to <sebastian.ramirezvillarreal@stonybrook.edu>. Thank you!\n";
...
static char   *instance_inname;          /* filename of ising instance file */

static char   *start_tour_filename;
...
static const char prog_buf[]="@(#)ising_sa version1.0 send";
...
/* set the version string */
      /*sprintf(version, "ising_sa version %s parallel", VERS);*/
...
/* following part parses command line for options and their arguments      */
  ...
  while( (c = getopt(argc, argv, OPTS)) != -1) {
    ...
    case 'b':
      brute_force=true; 
      /* -b sets backup frequency (to write state file) */
/*      state_write = strtol(optarg, NULL, 0);
      if ( state_write < 1 )
	error("ising_sa: max. backup frequency is every tau steps i.e. -b 1");
      if ( state_write == LONG_MAX )
        error("ising_sa: argument for -b too large"); */
      /*warning("ising_sa: state files not supported!");*/
      break;
    case 'c':               /* -c sets the frequency for printing captions */
      error("ising_sa: -c is not supported anymore, captions are off for good");
...
/*      captions = strtol(optarg, NULL, 0);
      if ( captions < 1 )
        error("ising_sa: can't print captions more than every line (-c 1)");
      if ( captions == LONG_MAX )
        error("ising_sa: argument for -c too large");                         */
      break;
...
/*    covar_index = atoi(optarg); 
      if ( covar_index < 1 )
        error("ising_sa: covariation sample index must be >= 1");            */
      error("ising_sa: -C does not work yet, try to run on fewer nodes");
      break;
	...
	error("ising_sa: valid stopping criteria are pfreeze, afreeze, abs");
      break;
    ...
    case 'f':
      ...
      if ( precision < 0 )
	error("ising_sa: what exactly would a negative precision be???");
      if ( precision > MAX_PRECISION )
	error("ising_sa: max. float precision is %d!", MAX_PRECISION);
      break;
    ...
    case 'Q':                  /* -Q sets quenchit mode (serial code only) */
      error("ising_sa: can't use -Q for parallel code");
      break;
    case 'r':
      break;
    case 's':
      init_point_flag=true;
      start_tour_filename=malloc(sizeof(*start_tour_filename)*MAX_RECORD);
      strcpy(start_tour_filename, optarg);
      break;
	...
	error("ising_sa: frequency of writing tune stats must be >= 1");
      break;
    case 'y':                             /* -y set frequency to print log */
      print_freq = strtol(optarg, NULL, 0);
      if ( print_freq < 1 )
	error("ising_sa: can't print status more than every tau steps (-y 1)");
      if ( print_freq < 1000 )
	  warning("ising_sa: writing log often (< 1000) slows down your code!");
      if ( print_freq == LONG_MAX )
	      error("ising_sa: argument for -y too large");
    break;
    case ':':
      error("ising_sa: need an argument for option -%c", optopt);
      break;
    case '?':
    default:
      error("ising_sa: unrecognized option -%c", optopt);
    }
  }
...
/* error checking here */
  ...
  if ( (tuning == 1) && (equil == 1) )
    error("ising_sa: can't combine -E with -T");
  if ( write_llog && !tuning )
    error("ising_sa: -L only makes sense when tuning");
  ...
  fscanf(fp,"%*s\n");    /* advance past title line no blanks allowed! */
   ...
   if ( 2!= (fscanf(fp,"%d %lf\n",  &(DistP.distribution), &(DistP.q) )) )
	...
	if ((DistP.distribution > 11) || (DistP.distribution < 1))
	  {error ("ising_sa: distribution must be int from 1 to 11 \n");}
	else if 
	  ((DistP.distribution == 5)||(DistP.distribution == 8)||(DistP.distribution == 10))
	  {error ("ising_sa: chose a distribution that returns positive values \n");
	  }
	else if (DistP.distribution == 7 )
          {  
	    if ((DistP.q >= 3.0) || (DistP.q <= 1.0))
	      {error ("ising_sa: q must be between 1 and 3 \n");}
	    else if (DistP.q == 2.0)
	      {DistP.distribution = 4;
	      /* ising needs abs lorentz, fly should be 5*/
	      printf ("ising_sa: q=2 is lorentz--setting distribution to 4\n");}
	    else if (DistP.q > 2.0)
	      {qgt2_init(); }
	    else 
	      {qlt2_init(); }
...
/* move generation (in move.c)                                             */
  ...
  if ( !instance_infile ) {
    ...
    exit(1);
                    ...
                    /* read input from ising lib file */
	           /* generate initial tour and get initial energy */
  InitIsing(instance_infile); /* this routine is in initialize.c */
  if (init_point_flag){
    FILE *start_tour_file = fopen(start_tour_filename,"r");
    *p_chisq = InitFromStartPoints(start_tour_file);
  } else {
    *p_chisq = RandStartingPoints(); /* Consistency */
  }
 ...
 * No, stats does not get allocated in StatRead.        */
  ...
  move_ptr = (MoveState *)malloc(sizeof(*move_ptr));
