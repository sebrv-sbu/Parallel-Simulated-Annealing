...
void StateRead(char *statefile, Opts *options, MoveState *moveptr,
...
//
  ...
  for(int i=0 ; i < n_points; i++){
    fscanf(infile, "%d ", &VERTEX_SPIN(i));
    }

  fscanf(infile,"\n");

  moveptr->acc_tab_ptr = (AccStats *)malloc(sizeof(AccStats));

  fscanf( infile, "%lg %c %c\n",
...
//
  for(int i=0; i < n_points; i++){
    fprintf(outfile, "%d ", VERTEX_SPIN(i));
  }
  fprintf(outfile,"\n");

  fprintf(outfile, "%.16g %c %c\n",
