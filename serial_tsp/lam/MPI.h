...
int myid, myid_forshmem;                   /* id of local node (processor) */
int my_group_id;                        /* group id of local node in group */

int ngroups;                                       /* number of CDR groups */
int glob_interval;             /* How many local mixes before a global mix */

/* nnodes is the number of processors actually annealing, P includes the   *
 * processors used for simulating shared memory, nodes_in_machine is used  *
 * for the shared memory processor to determine if the system has frozen   */
int nnodes, P, nodes_in_machine; 
...
int *root_ids;              /* array for root ids necessary for prolix and *
                             * equilibration runs                          */
int shmem_proc;       /* which processor is our "shared memory" processor? */
unsigned short shmem_flag;        /* flag set to 1 if we are shared memory */
