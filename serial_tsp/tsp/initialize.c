...
void GenSortedLists(){
  ...
  int k=0;
  coord i_coords;
  unsigned short num_shmem_procs;
  int *shmem_procs;

  distances = malloc(sizeof(dist_and_city)*(ncities-1));
  temp = malloc(sizeof(dist_and_city)*(ncities-1));
  sorted_lists=malloc((long)sizeof(int)*(long)ncities*(long)(ncities-1));
  

  for(i=0; i<ncities ; i++){
     MergeSort(i, &sorted_lists[(long)(i)*(long)(ncities-1)]);
   }
