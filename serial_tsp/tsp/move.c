...
static long int       nhits;    /* number of moves since the start of execution */
static long int       nsweeps;     /* number of sweeps since start of execution */
...
static int *curr_tour = NULL;  /* pointer to tour of city ids 
                                  in current order visited*/ 
static int *curr_position = NULL;  /* maintains the current tour position 
                               for each city*/
                    /*index is city id-1 element is curr_tour index */ 
static int swap[2];  /* array of indices of swapped elements in tour*/ 
...
{  /* begin tour_allocate */
 ...
 if (tour_debug) 
{ /* begin debug */
   min_tour = (int *) malloc (sizeof (int) * ncities);
   if (min_tour == NULL)
      return 1;
} /* end debug */
    return 0;
...
/* acc_tab is for statistics like acceptance ratio etc. */
  ...
  acc_tab = (AccStats *)malloc(sizeof(AccStats));
  acc_tab->acc_ratio = 0.;
 ...
 *** the address is what we need to index position                     *
************************************************************************/
...
{  /* begin accept_move*/
  unsigned short hold;                            /* temporary swap location */
 ...
 * according to the gain (see King-Wai's thesis, p. 23)                    */
  ...
  acc_tab->acc_ratio = 
    ((double)acc_tab->success)/((double)acc_tab->hits);

  /*   x  = log(acc_tab->theta_bar);  from fly/lj magnitude */
  x  = acc_tab->theta_bar;  /* tsp no log */

  x += ap.gain * (acc_tab->acc_ratio - 0.44);

   /*  acc_tab->theta_bar = exp(x);  from fly/lj magnitude */
  acc_tab->theta_bar = x;  /* tsp no exp  */
...
/* if -p: root node prints prolix information to prolix file */
  ...
  if ( prolix ) {
      fprintf(prolixptr, "nsteps = %8ld bar = %10.8e hits = %6d ",
	      nhits, acc_tab->theta_bar, acc_tab->hits ); 
      fprintf(prolixptr, "success = %6d acc_ratio = %5.2f\n", 
	      acc_tab->success, acc_tab->acc_ratio );
  }
...
void WriteResults(FILE *outptr, int precision)
{
  ...
  fprintf(outptr," obtained in %ld steps \n", nhits); 
...
int i;
fprintf(outfile,"\n annealing minimum cost is: %f", curr_cost); 
fprintf(outfile," obtained in %ld steps \n", nhits); 
...
/* calculate buffer size, compare with move parameters below */
  ...
  *lsize = 2*ncities+5; /* number of integer variables */
  *dsize = 3;               /* number of double variables */
...
/* pack longs into longbuf */
  ...
  (*longbuf)[0]   = (long)ncities; /* this should be the same on all nodes */
  (*longbuf)[1]   = (long)nhits;
  (*longbuf)[2]   = (long)nsweeps;

  (*longbuf)[3]   = (long)acc_tab->hits;
  (*longbuf)[4]   = (long)acc_tab->success;
  
   for ( i=0; i<ncities; i++)
            {/*begin  curr_tour and curr_position index from 0 to (ncities-1)*/
                (*longbuf)[i*2+5] = (long)curr_tour [i];
                (*longbuf)[i*2+6] = (long) curr_position[i];
           } /*end for */

 /* pack doubles into doublebuf */

  (*doublebuf)[0] = curr_cost;
  (*doublebuf)[1] = acc_tab->acc_ratio;
  (*doublebuf)[2] = acc_tab->theta_bar;
...
/* unpack longs */
  ...
  ncities      = (int)longbuf[0]; /* this should be the same on all nodes */
  nhits            = (int)longbuf[1];
  nsweeps          = (int)longbuf[2];

  acc_tab->hits    = (int)longbuf[3];
  acc_tab->success = (int)longbuf[4];

/* accept the current tour and the positions */

   for ( i=0; i<ncities; i++)
            {/*begin curr_tour and curr_position index from 0 to (ncities-1)*/
             curr_tour [i]    = (int)(longbuf)[i*2+5];
             curr_position[i] = (int)(longbuf)[i*2+6];
           } /*end */

/* unpack doubles */

  curr_cost          = doublebuf[0];
  acc_tab->acc_ratio = doublebuf[1];
  acc_tab->theta_bar = doublebuf[2];
...
***********************************************/
 double calc_new_cost(int *tour, int *swap, double original_cost)
...
MoveState *MoveSave(void){
  ...
  move_stuff->curr_tour     = (int *)calloc(ncities, sizeof(int));
  move_stuff->curr_position = (int *)calloc(ncities, sizeof(int));
