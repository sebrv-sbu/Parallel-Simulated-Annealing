...
void StateRead(char *statefile, Opts *options, MoveState *moveptr, double *stats,
  ...
  moveptr->curr_tour=(int *)calloc(moveptr->ncities, sizeof(int));
  moveptr->curr_position=(int *)calloc(moveptr->ncities, sizeof(int));
...
void StateWrite(char *statefile){
 ...
 if ( time_flag ) {
  ...
  fprintf(outfile, "%.16g %.16g %d %d\n",
    move_status->acc_tab_ptr->acc_ratio,
